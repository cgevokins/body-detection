...
class BodyPart {
    ...
    isHigherThan(otherPart) {
        if (this.hasConfidenceThreshold) {
            return this.confidenceScore > this.confidenceThreshold && otherPart.confidenceScore > this.confidenceThreshold && this.getY() < otherPart.getY()
        }
    }

    isLowerThan(otherPart) {
        if (this.hasConfidenceThreshold) {
            return this.confidenceScore > this.confidenceThreshold && otherPart.confidenceScore > this.confidenceThreshold && this.getY() > otherPart.getY()
        }
    }
    ...
    isLeftOf(otherPart) {
        if (this.hasConfidenceThreshold) {
            return this.confidenceScore > this.confidenceThreshold && otherPart.confidenceScore > this.confidenceThreshold && this.getX() > otherPart.getX()
        }
    }
...
class Body {
    ...
    getDistanceBetweenBodyParts (first, second) {
        ...
        if (firstBodyPart && secondBodyPart) {
            return Math.round(Math.hypot(firstBodyPart.position.x-secondBodyPart.position.x, firstBodyPart.position.y-secondBodyPart.position.y))
        } else {
            return 0
        }
    }

    getRelativeDistanceBetweenBodyParts(first, second) {
        ...
        return this.getDistanceBetweenBodyParts(first, second) / this.getDistanceBetweenBodyParts("leftEye", "rightEye") * 10
    }
}
